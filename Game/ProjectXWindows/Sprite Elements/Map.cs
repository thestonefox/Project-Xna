...
namespace ProjectXWindows.Sprite_Elements
{
    class Map
    {
        ...
        public void Render(SpriteBatch spriteBatch, EnemySpawner enemies)
        {
            SmoothScroll();
            ...
            for (int x = 0; x < xLoop; x++)
            {
                ...
                for (int y = 0; y < yLoop; y++)
                {
                    ...
                    for (int e = 0; e < tiles[x + tileOffset.X][y + tileOffset.Y].enemySpawns.Count; e++)
                    {
                        enemies.SpawnEnemy(tiles[x + tileOffset.X][y + tileOffset.Y].enemySpawns[e], new Vector2(xpos, ypos));
                    }
                    tiles[x + tileOffset.X][y + tileOffset.Y].enemySpawns.Clear();
                }
            }
        }
        ...
        {
            if (coodType == 'X') return (((tiles.Length) - (int)(drawArea.Width / tileSize.X)))-1;
            if (coodType == 'Y') return (((tiles[arraypos].Length) - (int)(drawArea.Height / tileSize.Y)))-1;
            return 0;
        }        
    }
}
