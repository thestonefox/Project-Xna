...
namespace ProjectXWindows.Game_Elements.Enemies
{
    class Shaker : Enemy
    {
        ...
        public override void SetTrajectory(float rotate)
        {
            ...
            trajectory[0] = new Vector4(startDirection.X, startDirection.Y, 225.0f, rotate);

            startDirection = new Vector2((float)Math.Sin(rotate), (float)-Math.Cos(rotate)) * acceleration;
            trajectory[1] = new Vector4(-startDirection.X, -startDirection.Y, 55.0f, rotate);

            startDirection = new Vector2((float)Math.Sin(rotate), (float)-Math.Cos(rotate)) * acceleration;
            trajectory[2] = new Vector4(startDirection.X, startDirection.Y, 55.0f, rotate);

            startDirection = new Vector2((float)Math.Sin(rotate), (float)-Math.Cos(rotate)) * acceleration;
            trajectory[3] = new Vector4(-startDirection.X, -startDirection.Y, 200.0f, rotate);

            startDirection = new Vector2((float)Math.Sin(rotate), (float)-Math.Cos(rotate)) * acceleration;
            trajectory[4] = new Vector4(startDirection.X, startDirection.Y, 0.0f, rotate);
        }
    }
}
