...
namespace ProjectXWindows.Game_Elements.Enemies
{
    class Squid : Enemy
    {
        ...
        {            
            ...
            trajectory[0] = new Vector4(startDirection.X, startDirection.Y, 25.0f, rotate);
            
            startDirection = new Vector2((float)Math.Sin(rotate), (float)-Math.Cos(rotate)) * (acceleration/5);
            trajectory[1] = new Vector4(-startDirection.X, -startDirection.Y, 10.0f, rotate);

            startDirection = new Vector2((float)Math.Sin(rotate), (float)-Math.Cos(rotate)) * acceleration;
            trajectory[2] = new Vector4(startDirection.X, startDirection.Y, 0.0f, rotate);
        }
    }
}
